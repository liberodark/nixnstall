...
pub enum NixstallError {
    ...
    #[error("Cannot read disk list from lsblk: {0}")]
    DiskProbe(String),
...
mod tests {
    ...
    fn config_error_display() {
        let err = NixstallError::Config {
            path: "/etc/nixnstall/installer.toml".to_string(),
            message: "missing flake".to_string(),
        };
        assert_eq!(
            err.to_string(),
            "Configuration error in /etc/nixnstall/installer.toml: missing flake"
        );
    }
