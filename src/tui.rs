    ...
    pub fn wizard(
        ...
        loop {
            ...
            self.terminal.draw(|frame| {
                ...
                frame.render_widget(footer(step, error.as_deref()), chunks[2]);
            })?;
            ...
            match (step, key.code) {
                (_, KeyCode::Esc) => match step {
                    ...
                    Step::Hostname => step = Step::Layout,
                ...
                (Step::Layout, KeyCode::Enter) => step = Step::Hostname,
                ...
                (Step::Confirm, KeyCode::Left) => profile_idx = profile_idx.saturating_sub(1),
                (Step::Confirm, KeyCode::Right) => {
...
fn footer(step: Step, error: Option<&str>) -> Paragraph<'static> {
    ...
    let hint = match step {
        Step::Disk | Step::Layout => "Up/Down select   Enter continue   Esc back",
        Step::Confirm => "Left/Right profile   Enter INSTALL   Esc back",
